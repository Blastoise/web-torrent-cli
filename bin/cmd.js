...
const { version: webTorrentCliVersion } = JSON.parse(
  fs.readFileSync(new URL("../package.json", import.meta.url))
);
const { version: webTorrentVersion } = JSON.parse(
  fs.readFileSync(
    new URL("../node_modules/webtorrent/package.json", import.meta.url)
  )
);
...
yargs
  ...
  .usage(
    fs
      .readFileSync(new URL("ascii-logo.txt", import.meta.url), "utf-8")
      .split("\n")
      .map(
        (line) =>
          chalk`{bold ${line.substring(0, 20)}}{red ${line.substring(20)}}`
      )
      .join("\n")
      .concat(
        "\n",
...
async function runDownload(torrentId) {
  ...
  torrent.on("done", () => {
    ...
    if (!argv.quiet) {
      ...
      console.log(
        chalk`\ntorrent downloaded {green successfully} from {bold ${numActiveWires}/${
          torrent.numPeers
        }} {green peers} in {bold ${getRuntime()}s}!`
      );
    }
    if (argv.onDone) {
  ...
  function onReady() {
    ...
    if (argv.select && typeof argv.select === "string") {
      customPlaylist = argv.select
        .toString()
        .split(",")
        .map((v) => {
          try {
            const temp = parseInt(v);
            if (!torrent.files[temp]) {
              return errorAndExit(`There's no file that maps to index ${v}`);
            }
            return temp;
          } catch (err) {
            return errorAndExit(err);
      ...
      onSelection(customPlaylist.join());
      return;
    }
  ...
  async function onSelection(index) {
    ...
    if (
      ...
      torrent.files.forEach((file, i) =>
        allHrefs.push(
          JSON.stringify(`${href}/${i}/${encodeURIComponent(file.name)}`)
        )
      );
      ...
      if (typeof index === "string") {
        argv.playlist = true;
        let checkPlaylist = [];
        checkPlaylist = index.split(",").map((v) => parseInt(v));
        index = checkPlaylist[0];
        checkPlaylist.forEach((val) => {
          allHrefs.push(
            JSON.stringify(
              `${href}/${val}/${encodeURIComponent(torrent.files[val].name)}`
            )
          );
        });
      } else {
...
function drawTorrent(torrent) {
  ...
  function draw() {
    ...
    line(
      chalk`{green ${seeding ? "Seeding" : "Downloading"}:} {bold ${
        torrent.name
      }}`
    );
    ...
    line(
      chalk`{green Running time:} {bold ${runtime}}  {green Time remaining:} {bold ${estimate}}  {green Peers:} {bold ${unchoked.length}/${torrent.numPeers}}`
    );

    if (argv.verbose) {
      line(
        chalk`{green Queued peers:} {bold ${torrent._numQueued}}  {green Blocked peers:} {bold ${blockedPeers}}  {green Hotswaps:} {bold ${hotswaps}}`
      );
    }
